...
public class HttpProxyChannelInitializer extends ChannelInitializer<SocketChannel> {
	...
	protected void initChannel(SocketChannel ch) throws Exception {
		HttpProxyClientHandler handler = appCtx.getBean(HttpProxyClientHandler.class, "task-" + taskCounter.getAndIncrement());
		ch.pipeline().addLast(new LoggingHandler(LogLevel.DEBUG), handler);
	}
}
