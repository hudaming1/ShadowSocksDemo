...
public class SimpleSocksServer {
	...
	private static void log(String str) {
		System.out.println("[" + Thread.currentThread().getName() + "]\t\t" + sdf.format(new Date()) + "\t\t" + str);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			ThreadPool.execute(new Runnable() {
				...
				public void run() {
					try {
						PipeSocket clientPipeSocket = new PipeSocket(socket);
						DataInputStream clientInputStream = new DataInputStream(clientPipeSocket.getInputStream());
						DataOutputStream clientOutputStream = new DataOutputStream(clientPipeSocket.getOutputStream());
						/**
						 * 接收第一次请求消息体
						 * 
						 * <pre>
						...
						if (atype == 1) {
							...
							serverHost = clientInputStream.read() + "." + clientInputStream.read() + "."
									+ clientInputStream.read() + "." + clientInputStream.read();
						} else if (atype == 3) {
						...
						log(String.format("handshake-2: version:%s, command:%s, rsv:%s, atype:%s, remoteServer:%s:%s",
								version, command, rsv, atype, serverHost, serverPort));

						// 和目标服务器建立连接
						PipeSocket serverPipeSocket = null;
						try {
							 serverPipeSocket = new PipeSocket(new Socket(serverHost, serverPort));
						...
						/**
						 * 打开传输管道，实现转发功能： 1.SocksServer读取本地请求 2.SocksServer转发到目标服务器
						 * 3.SocksServer读取目标服务器的响应 4.SocksServer将目标服务器响应转发给Local
						 * 
						 * <pre>
						 * 	+-------+            +-------------+          +--------+
						 *  | Local |  <---1---  | SimpleSocksServer | ---2---> | Remote |
						 *  +-------+            +-------------+          +--------+
						 *     ↑                    |      ↑                   |  
						 *     |                    |      |                   |
						 *     +---------4----------+      +--------3----------+
						 ...
						 */
						// pipe2
						ThreadPool.execute(new Pipe(serverPipeSocket, clientPipeSocket));
						// pipe1
						ThreadPool.execute(new Pipe(clientPipeSocket, serverPipeSocket));
					} catch (Exception ce) {
	...
	static class PipeSocket {
		
		private Socket socket;
		private volatile boolean inputStreamIsClosed = false;
		private volatile boolean outputStreamIsClosed = false;
		...
		public synchronized void markInputStreamClosed() {
			this.inputStreamIsClosed = true;
		}
		
		public synchronized void markOutputStreamClosed() {
			this.outputStreamIsClosed = true;
		}
		
		public synchronized void closeSocket() throws IOException {
			log("input:" + inputStreamIsClosed + "output:" + outputStreamIsClosed + ", socket:" + socket.isClosed());
			if (inputStreamIsClosed && outputStreamIsClosed) {
				socket.close();
	...
	static class Pipe implements Runnable {
		...
		private PipeSocket inputStreamSocket;
		private PipeSocket outputStreamSocket;
		private InputStream inputStream;
		private OutputStream outputStream;
		
		public Pipe(PipeSocket inputStreamSocket, PipeSocket outputStreamSocket) throws InstantiationException {
			this.inputStreamSocket = inputStreamSocket;
			this.outputStreamSocket = outputStreamSocket;
			try {
				this.inputStream = inputStreamSocket.getInputStream();
			} catch (Exception ce) {
				throw new InstantiationException("getInputStream[" + inputStreamSocket.getHost() + "] error");
			}
			try {
				this.outputStream = outputStreamSocket.getOutputStream();
			} catch (Exception ce) {
				throw new InstantiationException("getOutputStream[" + outputStreamSocket.getHost() + "] error");
			}
		}
		...
		public void run() {
			...
			try {
				outputStream.close();
				outputStreamSocket.markOutputStreamClosed();
				outputStreamSocket.closeSocket();
			...
			try {
				inputStream.close();
				inputStreamSocket.markInputStreamClosed();
				inputStreamSocket.closeSocket();
